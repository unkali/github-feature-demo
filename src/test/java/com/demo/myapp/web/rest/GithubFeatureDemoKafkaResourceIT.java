...
import com.demo.myapp.config.EmbeddedKafka;
import com.demo.myapp.config.KafkaSseConsumer;
import com.demo.myapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class GithubFeatureDemoKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/github-feature-demo-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/github-feature-demo-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/github-feature-demo-kafka/unregister"));
                return;
            }
        }
